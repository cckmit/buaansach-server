...
public class AreaResource {
    ...
    @PostMapping("/create")
    public ResponseEntity<AreaEntity> createArea(@Valid @RequestBody CreateAreaRequest request) {
        log.debug("REST request to create {} : {}", ENTITY_NAME, request);
        return ResponseEntity.ok(areaService.createArea(request));
    }

    @PutMapping("/update")
    public ResponseEntity<AreaEntity> updateArea(@Valid @RequestBody AreaEntity entity) {
    ...
    public ResponseEntity<List<AreaEntity>> getListAreaByStore(@PathVariable String storeGuid) {
